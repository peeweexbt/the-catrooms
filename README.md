...
1. If you don't already have one, create a free account at
   https://github.com.
2. On github.com, click **New repository** (top right → your avatar, or the
   green "New" button). Give it any name (e.g. `backrooms-cats`), leave it
   **Public** (GitHub Pages requires this on free accounts), and don't
   initialize it with a README (this folder already has one).
3. Back in Terminal, clear out one leftover artifact first (harmless, just
   from earlier setup), make the initial commit, and tell git to remember
   your GitHub login in macOS's Keychain (this matters later for the
   automated push — without it, every push would need an interactive login):
   ```
...
4. Connect it to the repo you just created (GitHub shows you this exact URL
   ...
   ```
   git remote add origin https://github.com/YOUR-USERNAME/backrooms-cats.git
   ...
   this; if asked for a token, create one at
   github.com → Settings → Developer settings → Personal access tokens).
   Because of the `credential.helper` line above, macOS saves this login in
   Keychain so future pushes (including automated ones) won't prompt again.
...
6. After a minute or two, GitHub shows you the live URL — something like
   `https://your-username.github.io/backrooms-cats/`.
...
3. At your registrar, add the DNS records GitHub's docs specify for apex/
   `www` domains (four `A` records pointing at GitHub's IPs, or a `CNAME`
   record for a `www` subdomain to `your-username.github.io`) — GitHub's
   Pages settings page links directly to the current instructions.
...
- `com.cats.backrooms.autopush.plist` — a macOS `launchd` job that runs
  that script at minute 25 of every hour (chosen to land after the Cowork
  hourly task has usually finished generating a new conversation).
...
or "no changes, skipping push" (normal if the hourly generation task
hasn't run yet, or already got pushed). If you see "PUSH FAILED", the most
likely cause is the cached credential expiring — running `git push` by hand
once in Terminal will show the real error and typically fix it by
re-caching a fresh login.
